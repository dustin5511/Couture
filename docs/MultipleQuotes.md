...
| 13 | `RecalcQuoteOnFreightChange`          | Update  | quote          | PostOperation 40  | –     | eb_shippingrateperhour, eb_cycletime, eb_trailerrateton, eb_straighttruckrateton                               | PostImage `PostImage`: eb_shippingrateperhour, eb_cycletime                                                                                  |
| 17 | `RecalcProjectFreightChange`          | Update  | opportunity    | PostOperation 40  | –     | eb_shippingrateperhour, eb_cycletime                                                                           | PostImage `PostImage`: eb_shippingrateperhour, eb_cycletime                                                                                  |
...
- `RecalcQuoteOnFreightChange` drives the per-quote freight cascade. If raw inputs change (shipping rate, cycle time) it recalculates the cached trailer / straight rates + total trip minutes and re-fires every child line. If only the rate fields change (manual override of trailer or straight rate) it skips the recalc so the manual values are preserved, and just re-fires the lines. Load and unload are hard-coded to 10 minutes — the plugin always uses the constants regardless of any stored field values. Self-recursion guarded via depth — the second-pass invocation triggered by our own rate write exits immediately. Freight is per-quote — no cross-quote cascade.
- `RecalcProjectFreightChange` does the same math on the parent Project as a **preview** when the user edits shipping rate or cycle time on the project. The calculated project rates feed new quotes via `ValidateFreightBeforeQuoteCreate`'s seed at quote Create — they do **not** cascade to existing quotes. Each quote owns its own freight from Create onward.
